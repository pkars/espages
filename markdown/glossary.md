...
-   [Amplify Damage](class/necromancer.html#amplify-damage)
-   [Ancient Decipherer][]
...
A cubing material to decipher (identify) an [Ancient Scroll][].
...
Ancient Scrolls need to be deciphererd before using. (See [Ancient Decipherer][].)
...
Anvil Stones are cubing materials used for Forging. They are made from
...
Automods are the new feature added in D2 Expansion. Similar to [staffmods](#staffmod),
...
Some spells can\'t be cast for a specific period after you cast that
...
-   See also [Marrowwalk Bug](#h8c78068).
...
[here](playing-tips.html#re2a1259).
...
from Gheed). More information [here](playing-tips.html#re2a1259).
...
-   abbreviation for [Dragon Stone][]
-   for the verb, see [D-Stoning][]
...
D-Stoning is enhancing an item through the use of [Dragon Stones][dragon stone]
...
requirement. [D-stoning recipies](http://esdb.d2se.org/es3cube_n.htm#dst)
...
Devil\'s Foods are the trigger item to cause the [World Event][]
...
Unique Rings. ([Diablo Clone][] is toned down in ES, both in life and drop,
so a cheaper trigger item is used.)
...
[World Event](#world_event). He looks like green colored Diablo
and uses the same attacks and spells, but is much stronger.
...
also used for [d-stoning][] existing items.
...
and [weapon speed modifiers](#wsm)
...
gain from items. If you then subtract the [WSM][]
...
::: {.ie5}
Table: EIAS/IAS Conversion Table
...
The Gem Can is one of the [stockers](#stocker),
...
dropped by monsters, but can be cubed from a [Null Rune][] and [crystals](#crystal).
...
-   **In Combat:** Not only the Attack Rate and Defense, but also the
...
-   **Item Drop:** If a monster drops an item, its Item Level (ilvl) is
    ...
    -   Unique and Set items have its own level, 10 for [Fallen Star](http://esdb.d2se.org/es3uarmo_n.htm)
...
The multi-stocker is one of the [Stockers](#stocker)
which is used for storing cubing materials, including Maple Leaves, [Dragon Stones][dragon stone],
...
Next Delay is an internal parameter of skills like [Cast Delay][].
...
::: {.ie5}
Table: Next Delay LIST(/data/global/excel/missiles.txt)
...
[Null Rune](playing-tips.html#o6232ff2) is needed to cube [Kanji runes][kanji rune].
...
Ores serve as containers for [crystals](#crystal).
...
no further use. More information [here](playing-tips.html#re2a1259).
...
Organs are cubing materials dropped by monsters. Specific monsters have
roughly 1/100 \~ 1/200 chance to drop a specific organ. Used for
[Tinkering] and other recipes. Also known as \"monsters\' body parts\".
...
Oskill suffixes (of Skill Master) are found at [Suffixes](http://esdb.d2se.org/es3suff_n.htm)
page of ES database.
...
1.  Player Killer.
2.  [The Phrozen Keep](https://web.archive.org/web/20201020204723/http://phrozenkeep.planetdiablo.gamespy.com/),
...
-   This mod doesn\'t prevent the heal ability of monsters (ex.
    Councils\' healing themselves and Zakarum Priests\' healing minions).
...
CtC suffixes (of Fire Ball, etc.) are found at [Suffixes](http://esdb.d2se.org/es3suff_n.htm)
page of ES database.
...
Transmuting junk items into a new item, like cubing 3 Magic Rings into a
Magic Amulet in the original D2. The name comes from \"rolling the dice again\".
...
jewels and charms are generally worth picking up in ES (unless you are super rich).
...
two [FAQ](es-faq.html#n0fac7d9)s for details.
...
Stockers are the items to store multiple items in 1x1 space. They are
...
::: {.ie5}
  Table: The cap of each item stored in stockers
...
A competition among Diablo II single players, introduced by diabloii.net
...
[Uber Organs](playing-tips.html#mc05cb2d) are used in certain [tinkering recipes](http://esdb.d2se.org/es3cube_n.htm#tin).
They can\'t be dropped by monsters. They are cubed from [crystals](#crystal) instead.
...
You can see the condition [here](tables.html#n7fdda65).
...
Weapon Speed Modifiers affect the speed at which a weapon can attack.
After applying diminishing returns to your item-based IAS, the WSM is
subtracted from your [EIAS][].
...
You can find the WSMs of all weapons in ES on the [weapons page](http://esdb.d2se.org/es3weap_n.htm)
at the ES database.
